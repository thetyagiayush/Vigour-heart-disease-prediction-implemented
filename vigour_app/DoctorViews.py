...
from vigour_app.models import Patients, LeaveReportDoctor, Doctors, FeedBackDoctors, CustomUser, PatientAppointment, \
    NotificationPatient, NotificationDoctors
...
def doctor_home(request):
    ...
    return render(request, "doctor_template/doctor_home_template.html",
                  {"notification_count": notification_count, "total_appointment": total_appointment,
                   "appointment_doctor": appointment_doctor, "leave_count": leave_count})
...
def doctor_apply_leave_save(request):
    if request.method != "POST":
        ...
        doctor_obj = Doctors.objects.get(admin=request.user.id)
        try:
            leave_report = LeaveReportDoctor(doctor_id=doctor_obj, leave_date=leave_date, leave_message=leave_msg,
                                             leave_status=0)
            leave_report.save()
